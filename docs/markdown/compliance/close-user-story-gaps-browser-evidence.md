...
**Date/build/commit** (identical for every row below — recorded once here rather than per row): 2026-08-22, base commit `1f685b6` (WU1–WU10), WU11 e2e specs run uncommitted on top, against the isolated local stack: backend `dev` profile → MariaDB 10.11 container `tuhospedaje-dev-db` on host port **3307** (never 3306), frontend `npm run dev` on `:5173`, seed migration `V1_9000__dev_demo_data.sql` (6 categories, 38 lodgings, admin `admin@tuhospedaje.com`).
...
| ID | Browser/Viewport | Route | Role/Fixtures | Stubs/Preconditions | Steps → Expected → Observed | Screenshot/trace | Overflow/KB/Touch | Provider | Result |
|---|---|---|---|---|---|---|---|---|---|
| BE-CAT-01 | Chromium desktop | `/administración` (Categories tab), `/` (public) | ADMIN (create/edit); anonymous (public render) | `AdminCategoriesPage.createCategory()` fills a deterministic local HTTPS fixture URL, not a real Cloudinary upload | Existing `admin-categories.spec.js` (unmodified) executed live for the first time this WU: create/edit/delete a category with the required `image-url` field, public `CategoryCard` renders the representative image with accessible alt text, category-filter click behavior unaffected | `e2e/playwright-report/` | N/A | Cloudinary — only a syntactically valid HTTPS URL fixture is used; no real upload/fetch occurs | **PASS** — 4/4 `chromium` |
...
## Known environment gap — pre-existing visual regression baselines

`e2e/tests/visual.spec.js` (unmodified by this change) fails 6/6 scenarios on **both** `chromium` and `firefox` (12 total), each with a small (0.01–0.02 ratio) pixel diff against its stored golden (`tests/visual.spec.js-snapshots/*-{chromium,firefox}-linux.png`). Two of the six failing pages — **login** and **register** — were not touched by any `close-user-story-gaps` work unit, yet fail with the same magnitude of diff as the changed pages (Home, ProductDetail, MyReservations, SearchResults). This is consistent with the goldens having been captured on a different machine/font-rendering environment than this session's, not with a functional regression from WU1–WU10/WU11. No goldens were regenerated in this pass (that would require touching `visual.spec.js`'s snapshot files, out of this WU's authorized scope); this is flagged here for the maintainer/orchestrator to decide whether to re-baseline in a separate, explicitly-authorized pass.
