...
- [x] Close the deferred WhatsApp open-redirect security finding.
  - Implementation: `WhatsAppButton.jsx` validates the fixed `https://wa.me` URL and renders `target="_blank"` with `rel="noopener noreferrer"`; no `window.open` remains.
  - Signed commits: implementation `6928067`; dependent tests `349cdfc`.
  - Review: native review lineages `review-ca9fcf726e096e65` and `review-079edb5325fbca8d` approved and acknowledged.
  - Validation: frontend coverage passed for 66 files / 621 tests. E2E was not run because it requires environment and external services.
