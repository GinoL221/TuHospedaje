...
test.describe("Home recommendations — stable pagination", () => {
  test("page 1 -> page 2 -> page 1 keeps the same identities and order", async ({
    page,
    homePage,
  }) => {
    await homePage.open("/");
    ...
    const page2Response = page.waitForResponse((response) => {
      ...
      return (
        url.pathname.includes("/lodgings/recommendations") &&
    ...
    await expect.poll(() => currentPageIds(page)).not.toEqual(page1First);
  ...
  test("last -> first navigation returns to the original first-page snapshot", async ({
    page,
    homePage,
  }) => {
    await homePage.open("/");
    ...
    await expect(
      page.getByRole("button", { name: "Última", exact: true }),
    ).toBeDisabled();
  ...
  test("explicit refresh replaces the snapshot from page 1 with a new revision request", async ({
    page,
    homePage,
  }) => {
    await homePage.open("/");
    ...
    const refreshRequest = page.waitForRequest(
      (req) =>
        req.url().includes("/lodgings/recommendations") &&
        req.url().includes("page=0"),
    );
    await page
      .getByRole("button", { name: "Actualizar recomendaciones" })
      .click();
  ...
  test("selecting a category does not call the recommendations endpoint", async ({
    page,
    homePage,
  }) => {
    await homePage.open("/");
    ...
    page.on("request", (req) => {
      if (req.url().includes("/lodgings/recommendations"))
    ...
    await expect(
      page.getByRole("button", { name: "Actualizar recomendaciones" }),
    ).toBeVisible();
  });

  test("a failed load shows an accessible error and a successful retry restores a valid page", async ({
    page,
    homePage,
  }) => {
    ...
    await expect(
      alert.getByText("No pudimos cargar las recomendaciones."),
    ).toBeVisible();
