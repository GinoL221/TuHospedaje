...
test.describe("ProductDetail — availability states", () => {
  ...
  test("shows loading then a ready usable state with zero occupied ranges", async ({
    page,
    authUser,
  }) => {
    /** @type {() => void} */
    ...
    await page.route(
      `**/lodgings/${LODGING_ID}/availability**`,
      async (route) => {
    ...
    await expect(
      page.locator('.availability-status[role="status"]'),
    ).toHaveText("Comprobando disponibilidad...");
    releaseAvailability();
    await expect(
      page.locator('.availability-status[role="status"]'),
    ).toHaveText("Todas las fechas están disponibles.");
    await expect(page.locator("#product-check-in")).toBeEnabled();
  });

  test("occupied ranges disable those dates and reserve stays blocked until a full valid range is chosen", async ({
    page,
    authUser,
  }) => {
  ...
  test("initial failure shows an accessible alert and retry recovers a ready state", async ({
    page,
    authUser,
  }) => {
...
test.describe("BookingPage — preflight and conflict recovery", () => {
  ...
  async function selectDate(page, pickerId, date) {
    ...
    const targetLabel = date.toLocaleDateString("en-US", {
      month: "long",
      year: "numeric",
    });
    for (let attempt = 0; attempt < 12; attempt += 1) {
      const header = (
        await popper.locator(".react-datepicker__current-month").innerText()
      ).toLowerCase();
  ...
  async function revealGuestPhone(page) {
    ...
    const hideDetails = page.getByRole("button", {
      name: "Ocultar detalles del huésped",
    });
  ...
  test("submit stays blocked until availability is ready, then a preflight conflict shows an inline alert with no navigation", async ({
    page,
    authUser,
  }) => {
    await page.route(
  ...
  test("a backend conflict on submit is the final authority and recovers availability for retry", async ({
    page,
    authUser,
  }) => {
    ...
    await page.route("**/reservations", async (route, request) => {
      ...
      await route.fulfill({
        ...
        contentType: "application/json",
        body: JSON.stringify({
          error: "El rango de fechas ya no está disponible.",
        }),
      });
    });
    ...
    const retryRequest = page.waitForRequest(
      (req) =>
        req.url().includes(`/lodgings/${LODGING_ID}/availability`) &&
        req.url().includes("checkIn"),
    );
    await page.getByRole("button", { name: "Confirmar reserva" }).click();

    await expect(page.locator('p.error[role="alert"]')).toHaveText(
      "El rango de fechas ya no está disponible.",
    );
