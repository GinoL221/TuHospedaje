...
describe("public shell contract", () => {
	...
	it("defines the global scroll and responsive clearance contract in layout.css", () => {
		...
		expect(rule(layoutCss, "main")).toMatch(
			/padding-top:\s*var\(--site-scroll-clearance\)/,
		);
	...
	it("loads shell styles in order and keeps admin on the separate guarded route", () => {
		expect(entry.indexOf('import "./App.css"')).toBeLessThan(
			entry.indexOf('import "./layout.css"'),
		);
		...
		expect(app).toMatch(
			/<Route element={<RequireAdmin \/>}>[\s\S]*path="\/administración"/,
		);
	});
