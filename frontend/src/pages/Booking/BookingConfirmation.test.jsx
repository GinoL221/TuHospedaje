...
describe("BookingConfirmationPage - with valid state", () => {
	it("shows the lodging name, formatted dates, guest name, and total price", () => {
		renderConfirmation({
			state: { reservation: reservationFixture, lodging: lodgingFixture },
		});
	...
	it("renders navigation links to /my-reservations and /", () => {
		...
		expect(
			screen.getByRole("link", { name: "Ver mis reservas" }),
		).toHaveAttribute("href", "/my-reservations");
		expect(
	...
	it("shows the reservation number and non-empty notes", () => {
		...
		expect(
			screen.getByText("Llegamos después de las 22:00"),
		).toBeInTheDocument();
	});
	...
	it("omits the notes section when notes are blank", () => {
		renderConfirmation({
			state: {
				reservation: { ...reservationFixture, notes: "   " },
