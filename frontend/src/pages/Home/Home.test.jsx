...
import { customRender, screen, waitFor, makeAuthValue } from "../../test/test-utils";
...
vi.mock("../../components/ProductCard/ProductCard", () => ({
  default: ({ lodging }) => <div data-testid="product-card">{lodging.name}</div>,
}));

const lodgingFixture = { id: 1, name: "Cabaña del Lago", pricePerNight: 100, imageUrls: [] };
...
function mockGetDefaults({
  ...
  get.mockImplementation((endpoint) => {
    if (endpoint.startsWith("/lodgings?category=")) return Promise.resolve(lodgings);
...
describe("Home - lodgings render", () => {
  it("renders lodging cards fetched on mount", async () => {
    ...
    expect(get).toHaveBeenCalledWith(expect.stringContaining("/lodgings?page="));
  });
  ...
  it("shows the empty state when there are no lodgings", async () => {
    ...
    expect(
      await screen.findByText("No hay alojamientos cargados todavía. Volvé más tarde.")
    ).toBeInTheDocument();
  });
});
...
describe("Home - category filter", () => {
  ...
  it("shows Mostrar todos when a category is active and hides it after deselection", async () => {
    ...
    expect(await screen.findByRole("button", { name: "Mostrar todos" })).toBeInTheDocument();

    await user.click(categoryBtn);
    await waitFor(() =>
      expect(screen.queryByRole("button", { name: "Mostrar todos" })).not.toBeInTheDocument()
    );
  });
});
