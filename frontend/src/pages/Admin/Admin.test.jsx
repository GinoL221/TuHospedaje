...
vi.mock("./AdminLodgings", () => ({
	default: () => <div data-testid="admin-lodgings" />,
}));
vi.mock("./AdminCategories", () => ({
	default: () => <div data-testid="admin-categories" />,
}));
vi.mock("./AdminFeatures", () => ({
	default: () => <div data-testid="admin-features" />,
}));
vi.mock("./AdminPolicies", () => ({
	default: () => <div data-testid="admin-policies" />,
}));
vi.mock("./AdminUsers", () => ({
	default: () => <div data-testid="admin-users" />,
}));
vi.mock("./AdminReservations", () => ({
	default: () => <div data-testid="admin-reservations" />,
}));
...
function renderAdmin({ authValue } = {}) {
	return customRender(
		...
		{
			authValue: authValue ?? makeAuthValue({ user: mockAdmin }),
			route: "/admin",
		},
	);
}
...
describe("Admin - mobile block", () => {
	...
	it("renders the mobile-unavailable message when the viewport is narrow", () => {
		...
		expect(
			screen.getByText("Panel no disponible en móvil"),
		).toBeInTheDocument();
		expect(screen.queryByTestId("admin-dashboard")).not.toBeInTheDocument();
	});
